...
export class UserNotesApi {
  ...
  async list(options: UserNoteListOptions) {
  ...
  async create(data: UserNoteCreateOrUpdateParams) {
  ...
  async update(id: number, data: UserNoteCreateOrUpdateParams) {
    const response = await this.request
...
const basePath = "/user_notes";

interface UserNoteCreateOrUpdateParams {
  author?: string;
  text: string;
  user: number;
  is_pinned?: boolean;
...
interface UserNoteListOptions extends BaseListOptions {
  author?: number;
